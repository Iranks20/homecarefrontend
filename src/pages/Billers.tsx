...
export default function Billers() {
  ...
  const handleAddBiller = async (billerData: Parameters<typeof userService.createUser>[0]) => {
    try {
      // Ensure role is set to biller
      const dataWithRole = { ...billerData, role: 'biller' as const };
                  ...
                  <tr key={biller.id} className="hover:bg-gray-50">
                    <td className="whitespace-nowrap px-6 py-4">
                      <div className="flex items-center">
                        {biller.avatar ? (
                          <img
                            src={biller.avatar.startsWith('http') 
                              ? biller.avatar 
                              : `${import.meta.env.VITE_API_URL?.replace('/api', '') || 'http://51.20.55.20:3007'}${biller.avatar.startsWith('/') ? biller.avatar : '/' + biller.avatar}`}
                            alt={biller.name}
